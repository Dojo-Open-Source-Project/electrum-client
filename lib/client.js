...
class Client {
	constructor(port, host, protocol, options, callbacks) {
		...
		this.afterConnect = (callbacks && callbacks.afterConnect) ? callbacks.afterConnect : null;
		this.afterClose = (callbacks && callbacks.afterClose) ? callbacks.afterClose : null;
		this.onLog = (callbacks && callbacks.onLog) ? callbacks.onLog : function(str) {
			console.log(str);
		};
	...
	response(msg) {
		...
		if (callback) {
			...
			this.log("Error getting callback while handling response.");
		}
	}
